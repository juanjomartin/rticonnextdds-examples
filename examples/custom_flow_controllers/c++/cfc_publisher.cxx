...
extern "C" int publisher_main(int domainId, int sample_count)
{
    ...
    /* If you want to change the QoS settings programmatically rather 
    * than using the XML file, you will need to uncomment the
    * commented sections in the file that modify them */
    ...
    /* By default, the transport builtin mask is set to UDPv4 and SHMEM. The flow
       controller consumes tokens for every transport enabled. This way, although
       data is not sent through UDPv4, if we are have the publisher and the subscriber 
       in the same machine, UDPv4 consumes tokens and so, it halves the effective
       send rate. To avoid this, we enable only one of them (in this example, UDPv4) */
    ...
    custom_fcp.token_bucket.bytes_per_token = 1100;
